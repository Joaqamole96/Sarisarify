...
>
	...
	>
		...
		<h2 class="mb-5 text-base font-bold text-gray-900">Confirm Sale</h2>

		<!-- Total row -->
		<div class="mb-4 flex items-center justify-between rounded-xl bg-gray-50 px-4 py-3">
		...
		{#if borrowAmount > 0}
			...
			{#if borrowerMode === 'existing'}
				{#if borrowers.list.length === 0}
					<p class="mb-4 rounded-xl bg-gray-50 px-4 py-3 text-sm text-gray-400">
						No borrowers yet. Use "Add New" to create one.
					</p>
				{:else}
					<div class="mb-4 max-h-40 overflow-y-auto rounded-xl border border-gray-100">
			...
			{#if borrowerMode === 'new'}
				<input
					...
					class="mb-4 w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3
			...
			{#if checkingBalance}
				<p class="mb-4 text-xs text-gray-400">Checking balance…</p>

<script lang="ts">
	...
	// Which sheet is open: 'add' | 'edit' | null
	...
	// Formatted price line shown on each product card.
	// fixed:    ₱35     or ₱35 / sachet
	// per_unit: ₱1.25/pc (ceil applied at sale time)
	function priceDisplay(p: Product): string {
		const amount = p.price % 1 === 0
			? `₱${p.price}`
			: `₱${p.price.toFixed(2)}`;
	...
	// Secondary badges shown under the price
...
<div class="flex h-full flex-col">
		...
		{#if products.list.length === 0}
			...
			<div class="flex flex-col items-center justify-center gap-3 px-8 py-24 text-center">
				...
				<p class="text-sm text-gray-400">
					Tap the <strong>+</strong> button below to add your first product.
				</p>
			</div>
		{:else}
			<ul>
				{#each products.list as product (product.id)}
					<li class="flex items-center gap-3 border-b border-gray-100 px-4 py-3">
						...
						>
							...
							<p class="text-sm text-green-700 font-medium">{priceDisplay(product)}</p>
						...
						<button
							...
							class="flex h-8 w-8 flex-shrink-0 items-center justify-center
								rounded-lg text-gray-300 active:text-red-400"
	...
	<button
		onclick={openAdd}
		class="absolute bottom-20 right-4 flex h-14 w-14 items-center justify-center
			rounded-full bg-green-600 text-white shadow-lg text-2xl active:bg-green-700"
		...
		<div class="w-full max-w-sm rounded-2xl bg-white p-6">
			...
			<div class="mt-5 flex gap-3">
				<button
					...
					class="flex-1 rounded-xl border border-gray-200 py-3 text-sm font-medium text-gray-700"
				>
					Cancel
				</button>
				<button
					onclick={confirmDelete}
					class="flex-1 rounded-xl bg-red-500 py-3 text-sm font-semibold text-white"
				>
					Delete
				</button>
			</div>
